...
export const POSTS: Post[] = [
  {
    slug: 'is-the-319m-solana-airdrop-real',
    title: 'Is the $319M Solana airdrop real?',
    description:
      'A $319M airdrop is trending for Solana users. It is real money and it is already yours — but it is not an airdrop, there is no snapshot, and nobody is eligible for anyone else’s share. Here is exactly what it is.',
    published: '2026-09-07',
    minutes: 6,
    blocks: [
      { kind: 'p', text: 'A post claiming a surprise $319M airdrop is coming to Solana users has been circulating widely. The money is real. The word "airdrop" is not.' },
      { kind: 'p', text: 'Nobody is giving anything away. What is happening is that Solana lowered the deposit required to keep an account open, and the difference belongs to whoever paid it. You are not being airdropped anything — you are being refunded.' },
      { kind: 'callout', label: 'THE SHORT ANSWER', text: 'Real money, wrong word. There is no snapshot, no eligibility list, no claim deadline and no token. You get back a portion of what you personally deposited, and nothing else.' },
      { kind: 'h', text: 'Where the number comes from' },
      { kind: 'p', text: 'Every account on Solana holds a refundable deposit called the rent-exempt minimum, calculated from its size:' },
      { kind: 'code', text: 'rent = (128 + data_len) × lamports_per_byte' },
      { kind: 'p', text: 'SIMD-0437 cuts lamports_per_byte from 6,960 to 696 across five separately gated stages. The formula never changes; only the rate falls. Once all five activate, roughly 3.08M SOL across the network sits above the new floor — about $307M at $99.60, which is where the headline figure comes from.' },
      { kind: 'p', text: 'A standard token account stores 165 bytes and is billed for 293. That gives the whole story in three lines:' },
      { kind: 'table', head: ['', 'Lamports/byte', 'Floor per token account', 'Refundable'], rows: [
        ['Legacy rate', '6,960', '0.00203928 SOL', '—'],
        ['Today (gate 1 live)', '6,333', '0.00185557 SOL', '0.00018371 SOL'],
        ['All five gates', '696', '0.00020393 SOL', '0.00183535 SOL'],
      ] },
      { kind: 'h', text: 'Why it is not an airdrop' },
      { kind: 'p', text: 'Airdrops have eligibility criteria, a snapshot block, and a fixed supply split among qualifying wallets. Farming one makes sense because your share depends on what you did before the cutoff.' },
      { kind: 'p', text: 'None of that applies here. Your refund is a function of one thing only: how many token accounts you personally opened and funded. Following an account, holding a badge, or being early changes nothing. There is no cutoff to beat and no way to increase your share except by having already paid more deposits.' },
      { kind: 'h', text: 'The part the viral posts leave out' },
      { kind: 'p', text: 'The 3.08M SOL figure covers every account on Solana — all 1.16 billion of them. Most of that is not in ordinary wallets. It sits in accounts owned by programs: the Token Program’s share is around 40%, with the rest spread across OpenBook, Serum V3, the BPF Upgradeable Loader, address lookup tables, Metaplex and the System Program.' },
      { kind: 'p', text: 'You can only reclaim deposits on accounts you control. So the honest per-person number is small: about 0.00018 SOL per token account today, rising to roughly 0.00184 once every gate is live. A wallet holding fifty token accounts is looking at a bit under a dollar now, and around ten dollars at full rollout.' },
      { kind: 'callout', label: 'WORTH SAYING PLAINLY', text: 'Nobody is receiving a share of $319M. You are receiving a share of what you deposited. For most wallets that is single-digit dollars today, and roughly ten times that once all five gates activate.' },
      { kind: 'h', text: 'Do you have to do anything?' },
      { kind: 'p', text: 'Yes. The surplus does not sweep itself back to you. It stays in the account until someone sends an instruction to move it, and no automatic refund is planned. Two instructions can retrieve it, and they are not equivalent.' },
      { kind: 'list', items: [
        'WithdrawExcessLamports takes only the surplus above the current floor. The account stays open and the tokens inside are untouched.',
        'CloseAccount returns the entire deposit but deletes the account, and it only works when the token balance is zero.',
      ] },
      { kind: 'p', text: 'For an account you still use, withdrawing is the one that makes sense. For an account left empty by a token you sold, closing returns more.' },
      { kind: 'h', text: 'How to check yours' },
      { kind: 'p', text: 'The floor is readable from any RPC with getMinimumBalanceForRentExemption, and comparing it against your accounts’ balances tells you the surplus. Overfunded does the comparison and builds the instruction, or you can query it directly:' },
      { kind: 'code', text: 'GET https://overfunded.app/api/v1/scan?wallet=YOUR_ADDRESS' },
      { kind: 'p', text: 'No key, no signup. The scan reads public chain state and signs nothing.' },
    ],
  },
  {
    slug: 'how-to-claim-your-share-of-the-solana-rent-refund',
    title: 'How to claim your share of the 3.08M SOL',
    description:
      'No snapshot, no eligibility, no claim page to farm. Your share of Solana’s rent refund depends only on the accounts you already own. Here is how to find it and how to get it back.',
    published: '2026-09-07',
    minutes: 5,
    blocks: [
      { kind: 'p', text: 'If you arrived here from a post about a $319M Solana airdrop, the useful thing to know is that there is nothing to qualify for. Your share is already sitting in your own accounts, and the only question is how to move it.' },
      { kind: 'h', text: 'Step one: find out whether you have any' },
      { kind: 'p', text: 'Every SPL token account you have ever opened holds a deposit. If it was opened before the rent reduction, part of that deposit is now surplus. If it was opened after, it is already at the floor and there is nothing to take.' },
      { kind: 'p', text: 'The check is arithmetic, not a lookup: compare each account’s lamport balance against the current rent-exempt minimum for its size. Anything above the floor is yours.' },
      { kind: 'code', text: 'GET https://overfunded.app/api/v1/scan?wallet=YOUR_ADDRESS' },
      { kind: 'h', text: 'Step two: pick the right instruction' },
      { kind: 'p', text: 'There are two, and choosing wrongly costs you either the account or the money.' },
      { kind: 'table', head: ['', 'WithdrawExcessLamports', 'CloseAccount'], rows: [
        ['Returns', 'Only the surplus above the floor', 'The entire deposit'],
        ['Account after', 'Still open and usable', 'Deleted'],
        ['Tokens inside', 'Untouched', 'Must already be zero'],
        ['Good for', 'Accounts you still use', 'Accounts left empty by a sale'],
      ] },
      { kind: 'p', text: 'Withdrawing is non-destructive: it moves the surplus and leaves everything else exactly as it was. Closing returns more per account, but only works on an empty one and the address is gone afterwards.' },
      { kind: 'h', text: 'Step three: sign it' },
      { kind: 'p', text: 'The instruction has to be signed by the account owner. Nothing sweeps automatically, and no service can move your lamports without your signature. Any site claiming to claim on your behalf without one is not doing what it says.' },
      { kind: 'callout', label: 'THERE IS NO CLAIM DEADLINE', text: 'The surplus does not expire and cannot be taken by anyone else. It sits in your account until you move it. Anything urging you to hurry before a deadline is inventing one.' },
      { kind: 'h', text: 'What to expect' },
      { kind: 'p', text: 'One gate of five is live today, so a token account currently holds about 0.00018 SOL of surplus. Fifty accounts is roughly 0.009 SOL. At full rollout the same fifty accounts are worth about 0.092 SOL, because the floor falls by 90% rather than 9%.' },
      { kind: 'p', text: 'That is the real shape of this: modest now, ten times larger later, and it applies to every account you open from here on.' },
      { kind: 'h', text: 'Doing it yourself' },
      { kind: 'p', text: 'The instruction is opcode 38 on both the Token and Token-2022 programs, taking three accounts: the source, the destination and the owner as signer. If you would rather build it yourself than use a site, the instructions are available pre-batched:' },
      { kind: 'code', text: 'GET https://overfunded.app/api/v1/withdraw-instructions?wallet=YOUR_ADDRESS' },
      { kind: 'p', text: 'It returns unsigned instructions grouped into transaction-sized batches. You sign and send them. No key is needed to call it.' },
    ],
  },
  {
